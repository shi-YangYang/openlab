...
openlab 将科研流程装进一个开箱即用的桌面工作台（Electron）：多平台文献搜索与下载、LLM 论文分析与综述、创新点与实验方案生成，以及一个能自主调用工具完成上述全流程的流式对话 Agent，并支持一键部署到远程 GPU 服务器（SSH + Web 终端）。
...
- LLM API Key 与 SSH 凭据只保存在你自己的电脑上，不会随代码上传或泄露。
- 下载的 PDF、数据库、会话记录等数据全部保存在本地。
- Agent 的危险操作（远程命令、代码执行等）必须在界面中逐次确认后才会执行。
...
从 [Releases](https://github.com/shi-YangYang/openlab/releases/latest) 下载 `openlab.Setup.*.exe`（Windows x64），双击安装即可。

安装版已内置全部运行环境，无需安装 Python / Node.js。
...
脚本会自动检测并安装缺失依赖，然后启动 openlab 桌面客户端。
...
- 四大平台并发搜索：arXiv、Semantic Scholar、百度学术、知网（浏览器登录态）。
- 「直接搜索」与「AI 智能搜索」两种模式；返回数量、分类、日期过滤。
- PDF 批量下载带进度条；本地存储、去重、删除时自动清理。
...
- 每个模型可单独设置上下文长度与思考强度，常见选项自动填充，也可手动调整。

### 科研 Agent

- 流式对话：回复逐字实时输出，每步工具调用的参数与结果可折叠查看，失败自动展开定位。
- 随时控制：运行中可一键停止，断线自动重连。
- 内置 32 个工具覆盖文献检索、论文分析、创新点、实验方案与执行、服务器管理、平台登录、命令与代码执行；危险操作强制人工审批。
- 上下文自动压缩：接近模型上下文窗口 80% 时摘要精简早期历史，保证长任务可持续。
- 会话自动保存：多会话管理、重命名、导出 Markdown、消息复制。
- 上下文用量圆环实时展示，超阈值变色预警。
...
- 训练在后台运行，可随时一键停止，防止占用 GPU；训练完成后自动结束并留存完整日志。
- 运行历史：列表、状态、耗时与日志回放。
...
| 打包分发 | PyInstaller（后端）+ electron-builder（NSIS 安装包） |
...
问题与反馈欢迎提交到 [GitHub Issue](https://github.com/shi-YangYang/openlab/issues)，接受 Pull Request。项目采用规格驱动开发（SDD），全部演进记录见 [specs/](specs/)；贡献前请阅读 [AGENTS.md](AGENTS.md) 了解开发流程与约定。
